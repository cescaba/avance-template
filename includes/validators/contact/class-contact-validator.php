...
class Avance_Contact_Validator {
	...
	public static function validate($data) {
		...
		} elseif (!self::verify_email_exists($data['email'])) {
			$errors[] = 'El email no existe o no es válido';
		}
		...
		if (isset($data['mensaje'])) {
			$msg_length = strlen(trim($data['mensaje'] ?? ''));
	...
	/**
	 * Verificar que el email existe (SMTP check básico)
	 */
	private static function verify_email_exists($email) {
		// Extraer dominio
		...
		// Fallback: validación básica de dominio
